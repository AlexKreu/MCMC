...
int main()
{
	...
	*/
	...
	/*
	int iter = 5000;
	Eigen::MatrixXd samples(iter, 2);
	std::vector<int> blocks_vector(2);
	blocks_vector.at(0) = 0;
	blocks_vector.at(1) = 2;
	//blocks_vector.at(2) = 2;
	samples = bm.sample_RWMH(iter, 23, blocks_vector);

	std::cout << "ll is:"  << samples.colwise().mean() << std::endl;

	return 0;

	double ss = sample_standard_normal();
	double s = sample_uniform();
	int d = 3;
	Eigen::VectorXd v(d);
	...
	{
		mean(i) = 10;
		v(i) = sample_standard_normal();
	}
	std::cout << "mean is:" << std::endl << mean << std::endl;

	Eigen::MatrixXf matA(d, d);

	double xs = sample_standard_normal();
	std::cout << "sample normal:" << std::endl << xs << std::endl;
	v(0) = sample_standard_normal();
	...
	double threshold = 0;
	if (log(sample_uniform()) < threshold)
	{
	   //accept new sample
	}
	*/
	...
	std::cout << "z ist " << z << "\n";
	int xx{12};
	std::cout << "Enter the value of xx \n";
	std::cin >> xx;
	std::cout << "you entered: " << xx;

	myMat M(5, 10);
	M.set_entry(1, 2, 1000);

	myMat X(5, 10);
	X.set_entry(1, 2, 1100);
	...
	std::cout << "Entry (1,2) is:" << Z.get_entry(1,2);
	//double value;
	//std::cout << "Enter value now\n";
	//std::cin >> value;
	*/
}
